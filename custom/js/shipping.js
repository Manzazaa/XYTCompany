...
$(document).ready(function() {
	...
	manageViewSuppPayments = $("#manageViewSuppPayments").DataTable({
		'ajax': 'php_action/fetchSP_shipping.php',
		'order': []		
	});
...
function editBrands(brandId = null) {
	if(brandId) {
		...
		$.ajax({
			url: 'php_action/fetchShipping.php',
			type: 'post',
			...
			success:function(response) {
				...
				$('#editBrandStatus').val(response.shipping_status);
				// brand id 
				$(".editBrandFooter").after('<input type="hidden" name="brandId" id="brandId" value="'+response.ordersupply_id+'" />');
