...
$sql = "SELECT ordersupply.ordersupply_id, ordersupply.order_date, brands.brand_name, ordersupply.total_amount, ordersupply.packaging_status, ordersupply.shipping_status, ordersupply.approval_status FROM ordersupply INNER JOIN brands ON ordersupply.brand_id = brands.brand_id WHERE ordersupply.shipping_status = 2";
 ...
 while($row = $result->fetch_array()) {
  ...
  if($row[6] == 1) {
    $approvalStatus = "<label class='label label-danger'>Pending</label>";
     ...
     $approvalStatus = "<label class='label label-success'>Done</label>";
  }
